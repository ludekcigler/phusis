...
function otf_videopreviewsrc($option_key, $result, $ext) {
	...
	$pattern = '/<(embed|iframe).+?src\s*=\s*[\'|\"](.*?)[\'|\"].+?>/i';
	if (!preg_match_all($pattern, $content, $matches)) {
		// no embed tags in content
		if (($s[3]) && ($s[4])) {
			// a default preview src has been given
			return $s[3].':'.$s[4];
		} else {
			return '';
		}
    ...
    $videoUrls = array('@http[s]?://www.youtube.com/v/([-_[:alnum:]]*)@',
                       ...
                       '@http://www.dailymotion.com/swf/video/([[:alnum:]]*)@',
                       '@http://dailymotion.virgilio.it/swf/video/([-_[:alnum:]]*)@',
                       '@http://www.dailymotion.com/video/([-_[:alnum:]]*)@',
                       '@http://www.dailymotion.com/embed/video/([-_[:alnum:]]*)@',
                       '@http://dailymotion.virgilio.it/embed/video/([-_[:alnum:]]*)@',
                       '@//www.dailymotion.com/embed/video/([-_[:alnum:]]*)@',
                       '@http[s]?://player.vimeo.com/video/([-_[:alnum:]]*)@');

    $videoPluginURL = plugin_dir_url(__FILE__).'video-plugins/';
    $previewUrls = array($videoPluginURL.'youtube.php?video=${VIDEO_ID}',
                         $videoPluginURL.'youtube.php?video=${VIDEO_ID}',
                         $videoPluginURL.'youtube.php?video=${VIDEO_ID}',
                         $videoPluginURL.'viddler.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'dailymotion.php?video=${VIDEO_ID}',
                         $videoPluginURL.'vimeo.php?video=${VIDEO_ID}');
    ...
    for ($i = 0; $i < count($videoUrls); ++$i) {
        if (preg_match($videoUrls[$i], $embedsrc, $matches)) {
            // Found video URL
            $videoId = $matches[1];

            $previewsrc = str_replace('${VIDEO_ID}', $videoId, $previewUrls[$i]);
            return $previewsrc;
        }
    }
...
function oth_trim_excerpt($content, $len) {
    ...
    $nodes = $xPath->query('//p[@class="audioplayer_container"]');
    if($nodes->item(0)) {
