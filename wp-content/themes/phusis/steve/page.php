...
<div id="content">
	...
	<?php if (have_posts()) : ?>
		...
		<?php while (have_posts()) : the_post(); ?>
                ...
                <?php if ( comments_open() ) : ?><p class="date"><?php comments_popup_link('Commentaires (<span class="commentcount">0</span>)', 'Commentaires (<span class="commentcount">1</span>)', 'Commentaires (<span class="commentcount">%</span>)'); ?></p><?php endif; ?>
  ...
  ?>
            ...
            if ($page_principale and function_exists('recent_posts')) {
                if ($page_principale == 'bienvenue') {
                    ...
                    recent_posts('limit=1&output_template=<a href="{url}"><img alt="{imagealt}" src="{imagesrc}" class="alignright size-medium" /></a>');
                ...
                $article_list_template = '<li><a href="{url}"><img alt="{imagealt}" src="{imagesrc}" height="48" width="64" /><span class="title">{title}</span> | <span class="date">{date}</span> | <span class="commentcount">({commentcount})</span><span class="chapeau">{excerpt:8:a:::1}...</span></a></li>';
